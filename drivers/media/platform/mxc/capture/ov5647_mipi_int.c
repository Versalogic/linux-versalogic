...
static void ov5647_set_virtual_channel(unsigned int channel)
{
	u8 channel_id;
			    ...
			    enum ov5647_mode mode, enum ov5647_mode orig_mode)
{
	...
	else if (ov5647_data.pix.pixelformat == V4L2_PIX_FMT_SBGGR8) 	/* reg 0x3034[3:0] == 0x8 is 8bit mode */
	...
	if (mipi_csi2_info) {
		...
		while (1) {
			...
			pr_debug("mipi_csi2 error1 = 0x%X\n", mipi_reg);
			if (!mipi_reg)
				break;
			if (i++ >= 20) {
...
static int ov5647_data_last_reg = 0;

static ssize_t show_reg(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	u8 val;
	s32 rval = ov5647_read_reg(ov5647_data_last_reg, &val);

	return sprintf(buf, "ov5647[0x%04x]=0x%02x\n",ov5647_data_last_reg, rval);
}
		       ...
		       const char *buf, size_t count)
{
	...
	if (2 == num_parsed) {
		...
		ov5647_write_reg(ov5647_data_last_reg, value);
	}
	return count;
}
...
static DEVICE_ATTR(ov5647_reg, S_IRUGO|S_IWUSR, show_reg, set_reg);
static DEVICE_ATTR(ov5647_mode, S_IRUGO|S_IWUSR, show_mode, set_mode);
